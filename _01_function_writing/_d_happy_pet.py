...
def feed(feed):
    messagebox.showinfo(title=None, message='your cat is happy now')
    return feed
if __name__ == '__main__':
    # TODO)
    #   1. Ask the user to enter the type of pet they want (give them a few
    #      choices).
    pet = simpledialog.askstring(title=None, prompt='what type of pet do you want: cat, dog, or fish')
    ...
    if pet == cat:
        while cat<happy:
            options = simpledialog.askstring(title=None, prompt='your cat is not happy! would you like to feed, walk, or play with it?')
            if options == 'feed':
                feed
